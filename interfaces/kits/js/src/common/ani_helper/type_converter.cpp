...
std::tuple<bool, std::optional<int32_t>> TypeConverter::ToOptionalInt32(ani_env *env, const ani_object &value)
{
    ...
    ani_int intValue;
    if (ANI_OK == env->Object_CallMethodByName_Int(value, BasicTypesConverter::toInt.c_str(), nullptr, &intValue)) {
...
std::tuple<bool, std::optional<int64_t>> TypeConverter::ToOptionalInt64(ani_env *env, const ani_object &value)
{
    ...
    ani_long longValue;
    if (ANI_OK == env->Object_CallMethodByName_Long(value, BasicTypesConverter::toLong.c_str(), nullptr, &longValue)) {
