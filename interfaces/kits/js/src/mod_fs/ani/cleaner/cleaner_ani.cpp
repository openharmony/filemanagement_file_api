...
void CleanerAni::Clean(ani_env *env, ani_object object)
{
    ...
    auto [succ, clsName] = TypeConverter::ToUTF8String(env, static_cast<ani_string>(nameObj));
    if (!succ) {
        HILOGE("Clean ParseString failed.");
        return;
    }
    if (clsName == "File") {
        HILOGE("Clean className: %{public}s", clsName.c_str()); 
        delete reinterpret_cast<FsFile *>(ptr);
    } else if (clsName == "RandomAccessFile") {
        HILOGE("Clean className: %{public}s", clsName.c_str()); 
        delete reinterpret_cast<FsRandomAccessFile *>(ptr);
    } else if (clsName == "ReaderIterator") {
        HILOGE("Clean className: %{public}s", clsName.c_str()); 
        delete reinterpret_cast<FsReaderIterator *>(ptr);
    } else if (clsName == "Stat") {
        HILOGE("Clean className: %{public}s", clsName.c_str()); 
        delete reinterpret_cast<FsStat *>(ptr);
    } else if (clsName == "Stream") {
        HILOGE("Clean className: %{public}s", clsName.c_str()); 
        delete reinterpret_cast<FsStream *>(ptr);
    } else if (clsName == "AtomicFile") {
        HILOGE("Clean className: %{public}s", clsName.c_str()); 
        delete reinterpret_cast<FsAtomicFile *>(ptr);
    } else {
        HILOGE("Clean unsupport className: %{public}s", clsName.c_str());
    }
    return;
}
