...
{
    unique_ptr<uv_fs_t, decltype(FsUtils::FsReqCleanup) *> openReq = { new (std::nothrow) uv_fs_t,
...
FsResult<FsFile *> OpenCore::DoOpen(const string &path, const optional<int32_t> &mode)
{
    ...
    if (path.find("://") != string::npos) {
        ...
        return FileInstantiator::InstantiateFile(res, uriStr, true);
    }
#endif
    ...
    return FileInstantiator::InstantiateFile(ret, path, false);
}
