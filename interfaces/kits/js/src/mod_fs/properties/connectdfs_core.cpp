...
FsResult<void> ConnectDfsCore::ConnectDfsExec(const std::string &networkId, sptr<FileDfsListenerStub> listener)
{
    if (networkId == "" || listener == nullptr) {
        return FsResult<void>::Error(E_PARAMS);
    }

    int result = Storage::DistributedFile::DistributedFileDaemonManager::GetInstance().
            OpenP2PConnectionEx(networkId, listener);
