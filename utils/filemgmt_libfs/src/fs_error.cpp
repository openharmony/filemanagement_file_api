...
std::unordered_map<int, int> softbusErr2ErrCodeTable {
...
std::unordered_map<std::string_view, int> uvCode2ErrCodeTable {
    { "EPERM", EPERM },
    { "ENOENT", ENOENT },
    { "ESRCH", ESRCH },
    { "EINTR", EINTR },
...
std::unordered_map<int, std::pair<int32_t, std::string>> errCodeTable {
...
FsError::FsError(int errCode, const std::string &errMsg)
{
    int genericCode = ConvertUVCode2ErrCode(errCode);
    auto it = errCodeTable.find(genericCode);
    if (it != errCodeTable.end()) {
